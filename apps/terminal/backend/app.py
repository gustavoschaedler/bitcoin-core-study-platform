...
BITCOIND_PORT = int(os.getenv("BITCOIND_PORT", "38332"))
BITCOIND_USER = os.getenv("BITCOIND_USER", "bitcoin")
BITCOIND_PASS = os.getenv("BITCOIND_PASS", "bitcoin")
# Two RPC auth modes mirroring the rest of the stack:
#   - "password" (default): use BITCOIND_USER / BITCOIND_PASS.
#   - "cookie": read user:pass from the bitcoind-generated cookie file.
RPC_AUTH_MODE = os.getenv("BITCOIN_RPC_AUTH_MODE", "password").strip().lower()
BITCOIND_COOKIE_FILE = os.getenv(
    "BITCOIN_RPC_COOKIE_FILE",
    "/bitcoind-data/signet/.cookie" if RPC_AUTH_MODE == "cookie" else "",
)
...
app = FastAPI(
    title="Bitcoin Core Terminal — Signet",
    version="0.1.0",
    docs_url="/api",
...
def get_rpc_auth() -> tuple[str, str]:
    ...
    Honours ``BITCOIN_RPC_AUTH_MODE``:
      - ``password`` (default): always use BITCOIND_USER / BITCOIND_PASS.
      - ``cookie``: read the cookie file; only if reading fails AND password
        credentials are populated do we fall back, logging a warning.

    In password mode the cookie file is never read, even if the env var points
...
async def exec_cmd(req: ExecRequest) -> dict[str, Any]:
    ...
    # Restrict cwd to a small allow-list so a caller cannot chdir into a
    # sensitive container path before the shell runs.
    safe_cwd = None
    if req.cwd:
        candidate = os.path.realpath(req.cwd)
        allowed = (
            os.path.realpath(os.environ.get("HOME", "/home/sandbox")),
            "/tmp",
            "/app",
        )
        if not any(
            ...
            raise HTTPException(
                status_code=400,
                detail={"code": "exec_cwd_forbidden"},
            )
        safe_cwd = candidate
