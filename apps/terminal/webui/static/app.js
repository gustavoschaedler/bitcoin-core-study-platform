...
class Pane {
  ...
  _formatRpcDisplay(line) {
    const t = (line || '').trim();
    if (!t) return '';
    return t.startsWith('bitcoin-cli') ? t : `bitcoin-cli -signet ${t}`;
  }
  ...
  _formatRpcRequestDisplay(req) {
    const parts = ['bitcoin-cli', '-signet'];
  ...
  async _runCommand(rawLine) {
    ...
    try {
      ...
      if (req.generateN !== null) {
        ...
        const display = `bitcoin-cli -signet -rpcwallet=${this._stringifyCliArg(req.wallet)} generatetoaddress ${this._stringifyCliArg(req.generateN)} ${this._stringifyCliArg(addr)}`;
