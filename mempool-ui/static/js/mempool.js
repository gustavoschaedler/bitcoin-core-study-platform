...
function renderEdgeHash(value, className = "hash-link"){
  ...
  return `<span class="${className}" title="${safe}"><span class="hash-edge">${head}</span><span class="hash-middle">${middle}</span><span class="hash-edge">${tail}</span></span>`;
}
...
function renderIoRow(item, type){
  ...
  const reference = type === "input" && txid
    ? `${renderIoTxRef(txid)}<span class="io-ref-index">:${escapeHtml(index ?? "-")}</span>`
    : `#${escapeHtml(index ?? "-")}`;
  const meta = type === "input"
    ? `${tr("sequence")} ${item.sequence ?? "-"} · ${tr("witness")} ${item.witness_items ?? 0}`
    : `${tr("address_type")} ${item.type || "-"} · ${tr("output_index")} #${item.n ?? "-"}`;
  return `<div class="io-card ${type === "input" ? "io-input" : "io-output"}">
    <div class="io-card-main">
      <div class="io-line">
        <span class="io-caption">${escapeHtml(type === "input" ? tr("previous_output") : tr("output_index"))}</span>
...
function renderTxDetail(tx){
  ...
  <section class="tx-hash-card">
    <span>TXID</span>
    <a href="${explorer}" target="_blank" rel="noopener noreferrer">${escapeHtml(tx.txid)}</a>
  </section>
...
async function openTxDetail(txid){
  ...
  $("tx-detail-title").textContent = txid;
  ...
  try{
    ...
    $("tx-detail-title").textContent = tx.txid;
        ...
        return `<button class="utxo-bubble coin-svg${index === rotatingIndex ? " coin-spin-once" : ""}" type="button" data-txid="${escapeHtml(utxo.txid || "")}" style="--bubble-size:${size}px">
          <span class="coin-shell">
            <span class="coin-side coin-front">
              <img class="coin-line-art" src="/mempool-assets/img/abstract-linear-bitcoin-coin.svg" alt="" aria-hidden="true">
